...
export function VoiceOverlay({ isOpen, onClose, sendMessage }: VoiceOverlayProps) {
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0, y: 20 }}
          className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-paper-100/95 backdrop-blur-xl"
        >
          {/* Header Controls */}
          <div className="absolute top-6 right-6">
            <button
              onClick={onClose}
              className="p-4 rounded-full bg-stone-200 text-stone-600 hover:bg-stone-300 transition-colors"
            >
              <X size={24} />
            </button>
          </div>
          ...
          <div className="relative flex items-center justify-center w-64 h-64">
             ...
             <motion.div 
               ...
               className={`absolute inset-0 rounded-full opacity-20 ${visualizerColor}`}
             />
             
             {/* Core Orb */}
             <motion.div
                animate={{ scale: visualizerScale }}
                className={`w-32 h-32 rounded-full shadow-2xl transition-colors duration-300 ${visualizerColor}`}
             />
             
             {/* Status Text */}
             <div className="absolute -bottom-16 text-stone-500 font-serif text-lg tracking-wide">
          ...
          {/* Live Transcript */}
          <div className="mt-24 h-24 px-8 text-center max-w-2xl">
             <AnimatePresence mode="wait">
                {transcript && (
                  <motion.p 
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: -10 }}
                    className="text-2xl font-serif text-stone-800 leading-relaxed"
                  >
                    "{transcript}"
                  </motion.p>
                )}
          ...
          <div className="absolute bottom-12 flex gap-8">
             <button 
               onClick={toggleMute}
               className={`p-6 rounded-full transition-all duration-300 ${isMuted ? 'bg-red-100 text-red-600' : 'bg-white text-stone-800 shadow-lg'}`}
             >
