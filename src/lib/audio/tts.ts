...
export class SynthesizerService {
    private static tts: any | null = null; // KokoroTTS
    private static modelId = "onnx-community/Kokoro-82M-ONNX";
    private static dtype: "fp32" | "fp16" | "q8" | "q4" | "q4f16" = "fp32";
    ...
    static async init() {
         ...
         if (!hasCloud) {
             ...
             if (!this.tts) {
                 ...
                 let transformers = (globalThis as any).transformers;
                 if (!transformers) {
                     // @ts-ignore
    ...
    static async speak(text: string): Promise<{ audio: Float32Array; sampling_rate: number } | null> {
