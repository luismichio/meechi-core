 ...
 */
export function oklchStringToHex(raw) {
 ...
 */
export function labStringToHex(raw) {
    ...
    // Lab → XYZ (D50)
    const δ = 6 / 29;
    const finv = (t) => (t > δ ? t ** 3 : 3 * δ * δ * (t - 4 / 29));
    const fy = (L + 16) / 116;
 ...
 */
export function anyColorStringToHex(s) {
    const t = s.trim();
    if (!t || t === 'transparent')
        return '';
    // #rrggbb or #rgb
 ...
 */
export function resolveTokenToHex(varName) {
