...
export class LocalStorageProvider {
    ...
    async indexFile(path, content) {
        ...
        try {
            ...
            const fileUpdatedAt = (_b = file === null || file === void 0 ? void 0 : file.updatedAt) !== null && _b !== void 0 ? _b : Date.now();
            let fullText = content;
            if (comments.length > 0) {
                const commentText = comments
                    .filter((c) => { var _a; return (_a = c.text) === null || _a === void 0 ? void 0 : _a.trim(); })
                    .map((c) => c.text)
                    .join('\n');
                if (commentText) {
                    fullText += "\n\n### User Notes & Comments\n" + commentText;
        ...
        if (query) {
            ...
            try {
                ...
                const matchedFiles = allFiles.filter(f => {
                    ...
                    if (queryLower.length > 3 && filenameNoExt.includes(queryLower))
                        return true;
                    if (filenameNoExt.length > 3 && queryLower.includes(filenameNoExt))
                        return true;
                    return false;
                });
