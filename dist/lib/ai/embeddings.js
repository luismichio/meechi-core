...
export function chunkText(text, maxChunkSize = 1000, overlap = 200) {
    ...
    const splitSection = (section) => {
        ...
        for (const line of lines) {
                    ...
                    if ((currentChunk + '\n' + codeFenceBuffer).length <= maxChunkSize) {
                        ...
                        if (codeFenceBuffer.length > maxChunkSize) {
                            ...
                            while (start < codeFenceBuffer.length) {
                                const end = Math.min(start + maxChunkSize, codeFenceBuffer.length);
                                pushChunk(headerPrefix + codeFenceBuffer.slice(start, end));
                                start += maxChunkSize - effectiveOverlap;
                                if (end === codeFenceBuffer.length)
                                    break;
                            }
                            currentChunk = headerPrefix;
                        }
                ...
                if (currentChunk.trim()) {
                    if (currentChunk.length > maxChunkSize) {
                        ...
                        while (start < currentChunk.length) {
                            const end = Math.min(start + maxChunkSize, currentChunk.length);
                            pushChunk(currentChunk.slice(start, end));
                            start += maxChunkSize - effectiveOverlap;
                            if (end === currentChunk.length)
                                break;
                        }
                        currentChunk = ''; // Hard reset after cut
                    }
                    else {
                        // Try to accumulate — will flush on next para if needed
                    }
                }
                // Only add separator if we have content
                if (currentChunk.trim()) {
                    currentChunk += '\n\n';
                }
                continue;
            ...
            if (candidate.length > maxChunkSize && currentChunk.trim()) {
                ...
                while (start < line.length) {
                    const end = Math.min(start + maxChunkSize, line.length);
                    pushChunk(headerPrefix + line.slice(start, end));
                    start += maxChunkSize - effectiveOverlap;
                    if (end === line.length)
                        break;
                }
        ...
        if (inCodeFence && codeFenceBuffer.trim())
            pushChunk(headerPrefix + codeFenceBuffer);
        // Final flush: only if content exists beyond the header prefix
        const finalTrimmed = currentChunk.trim();
        if (finalTrimmed && finalTrimmed !== headerPrefix.trim()) {
            pushChunk(currentChunk);
        }
    };
