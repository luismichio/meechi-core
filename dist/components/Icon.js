...
export default function Icon(_a) {
    ...
    const [library, setLibrary] = React.useState('default');
    React.useEffect(() => {
        async function load() {
            var _a;
            const cfg = await settingsManager.getConfig();
            const lib = (_a = cfg.appearance) === null || _a === void 0 ? void 0 : _a.iconLibrary;
    ...
    if (library === 'bold') {
        return _jsx(LucideIcon, Object.assign({ size: size, className: className, color: color, strokeWidth: 2.8, strokeLinejoin: "round", strokeLinecap: "round" }, props));
    }
    if (library === 'thin') {
        return _jsx(LucideIcon, Object.assign({ size: size, className: className, color: color, strokeWidth: 0.75, strokeLinejoin: "round", strokeLinecap: "round" }, props));
    }
    // Default Lucide
